  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 pt-24 pb-24">
      <div className="flex items-end justify-between gap-4 flex-wrap mb-8 cl-rise">
        <div>
          <div className="cl-label mb-2">Community · Public Gallery</div>
          <h1 className="font-display font-bold text-slate-100 text-3xl sm:text-4xl lg:text-5xl tracking-tight">
            Every recipe, ever analyzed.
          </h1>
          <p className="mt-3 text-slate-400 max-w-xl">
            Browse frames the community has decoded. Tap any card to inspect the
            full technical readout and copy the grade.
          </p>
        </div>
...
function GalleryCard({ item, idx }) {
  const t = item.analysis?.technical || {};
  return (
    <Link
      to={`/analysis/${item.id}`}
    ...
    >
      ...
      <div className="p-4">
        <div className="font-display text-slate-100 text-base truncate">
          {item.analysis?.title || "Untitled Frame"}
        </div>
        <div className="mt-1 flex items-center gap-1.5 text-xs text-slate-400 truncate">
          ...
          <span className="truncate">{t.device || "Unknown device"}</span>
        </div>
  ...
  return (
    <div className="cl-card p-16 text-center" data-testid="gallery-empty">
      ...
      <p className="text-sm text-slate-400 mb-6">
        Be the first to analyze a frame — every result is added here.
      </p>
      <Link to="/">
